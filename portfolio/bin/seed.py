#!/usr/bin/env python3
"""Incept the model portfolios. Safe to run again: a book already incepted is skipped.
...
--date), the three-year history in data/financials/style_history.csv, the stored closes in docs/prices/ (restored from gh-pages by the
...
def main(argv=None):
    ...
    for bid in out["seeded"]:
        picks = out["books"].get(bid)
        if picks is None:
            print(f"{bid}: $1,000,000 in cash; the PM builds it.")
        else:
            print(f"{bid}: {len(picks)} holdings; top 5 by rank: "
                  + ", ".join(p["ticker"] for p in picks[:5]))
    for bid, why in out["skipped"].items():
        print(f"{bid}: not seeded, {why}. Run this again once the history is in.")
    return 0
