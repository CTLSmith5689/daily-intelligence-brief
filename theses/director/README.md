...
`inputs/{WEEK_OF}.json` and `.md` are what `theses/bin/director_inputs.py` showed the director that week, committed with the plan so any plan can be read against its inputs.
