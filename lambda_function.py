...
.stk-map-h { display:flex; align-items:baseline; justify-content:space-between; gap:12px;
  padding:0 0 9px; margin-bottom:11px; border-bottom:1px solid var(--text-1);
  font-family:'Space Mono',monospace; font-size:10px; letter-spacing:2px; text-transform:uppercase; }
.stk-map-hint { font-size:9px; letter-spacing:1.2px; color:var(--text-4); text-transform:none; }
...
.stk-map-plot { position:relative; flex:1; min-height:420px;
...
.stk-radar { display:grid; grid-template-columns:repeat(3, minmax(0,1fr));
  grid-template-rows:auto auto; align-items:start; gap:0 22px;
  align-items:start; justify-content:start; }
.stk-radar > .stk-map { grid-column:1; grid-row:1 / span 2; }
.stk-radar > .stk-radar-plot { grid-column:2; grid-row:1; }
.stk-radar > .stk-radar-note { grid-column:2; grid-row:2; }
.stk-radar > .stk-radar-note { grid-column:1; grid-row:2; max-width:720px; }
.stk-radar > .stk-radar-side { grid-column:3; grid-row:1 / span 2; }
...
.stk-picklist { border-top:1px solid var(--text-1); max-height:min(72vh, 760px); overflow-y:auto; }
...
@media (max-width:1200px) {
  ...
  .stk-radar > .stk-map, .stk-radar > .stk-radar-plot,
...
(function() {
  ...
  function renderPickList() {
    ...
    rows = rows.slice().sort(function(a, b) {
      const av = computeComposite(a), bv = computeComposite(b);
      if (av == null && bv == null) return 0;
      if (av == null) return 1;
      if (bv == null) return -1;
      return bv - av;
    ...
    el.innerHTML =
      ...
      (rows.length > PICKLIST_MAX ? ', top ' + PICKLIST_MAX + ' by score' : '') + '</div>' +
  ...
  (function wireMapClick() {
    ...
    const tip = document.getElementById('stk-radar-note');
        ...
        <div id="stk-radar" class="stk-radar" hidden>
          <div class="stk-map">
            <div class="stk-map-h">
              ...
              <span class="stk-map-hint">Click to focus &middot; ctrl-click to add &middot; drag to turn
                <button type="button" class="stk-map-spin" id="stk-spin">Spin</button></span>
            </div>
